#!/usr/bin/env python3
"""
AgriSync 360 Complete QA Verification
Tests every endpoint comprehensively
"""
...
def test_authentication():
    ...
    log_test("1.1 Register farmer", success, f"Status: {r.status_code}")
    results["1.1"] = success
    ...
    if r.status_code in [200, 201]:
        ...
        log_test("1.2 OTP returned", otp is not None)
        results["1.2"] = otp is not None
    
    # 1.3 Verify OTP or login
    if otp:
        r2 = requests.post(f"{BASE_URL}/api/auth/verify-otp", json={
            "phone": "0711111111", "otp": str(otp)
        })
        success = r2.status_code == 200
        log_test("1.3 Verify OTP", success, f"Status: {r2.status_code}")
        results["1.3"] = success
        ...
        success = r2.status_code == 200
        log_test("1.3 Login existing", success, f"Status: {r2.status_code}")
        results["1.3"] = success
        FARMER_TOKEN = r2.json().get('data', {}).get('access_token', '')
    ...
    r = requests.post(f"{BASE_URL}/api/auth/register", json={
        "phone": "0722222222", "password": "TestPass1!", "role": "agro_dealer"
    })
    if r.status_code in [200, 201]:
        otp = r.json().get('data', {}).get('otp')
        r2 = requests.post(f"{BASE_URL}/api/auth/verify-otp", json={
    ...
    log_test("1.5 Agro-dealer registered", success)
    results["1.5"] = success
    ...
    # 1.6 Register NGO
    r = requests.post(f"{BASE_URL}/api/auth/register", json={
        "phone": "0733333333", "password": "TestPass1!", "role": "ngo_partner"
    })
    if r.status_code in [200, 201]:
        otp = r.json().get('data', {}).get('otp')
        r2 = requests.post(f"{BASE_URL}/api/auth/verify-otp", json={
    ...
    log_test("1.6 NGO registered", success)
    results["1.6"] = success
    ...
    # 1.7 Register Admin
    r = requests.post(f"{BASE_URL}/api/auth/register", json={
        "phone": "0700000001", "password": "Admin1234!", "role": "admin"
    })
    if r.status_code in [200, 201]:
        otp = r.json().get('data', {}).get('otp')
        r2 = requests.post(f"{BASE_URL}/api/auth/verify-otp", json={
    ...
    log_test("1.7 Admin registered", success)
    results["1.7"] = success
...
def test_password_reset():
    ...
    log_test("2.1 Forgot password request", success, f"Status: {r.status_code}")
    results["2.1"] = success
    
    reset_otp = r.json().get('data', {}).get('otp')
    log_test("2.2 Reset OTP returned", reset_otp is not None)
    results["2.2"] = reset_otp is not None
    
    if reset_otp:
        # 2.3 Verify reset OTP
        r = requests.post(f"{BASE_URL}/api/auth/verify-reset-otp", json={
            "phone": "0711111111", "otp": str(reset_otp)
        })
        success = r.status_code == 200
        log_test("2.3 Verify reset OTP", success, f"Status: {r.status_code}")
        results["2.3"] = success
        ...
        if reset_token:
            ...
            r = requests.post(f"{BASE_URL}/api/auth/reset-password", json={
                "reset_token": reset_token, "new_password": "NewPass1234!"
            })
            success = r.status_code == 200
            log_test("2.5 Reset password", success, f"Status: {r.status_code}")
            results["2.5"] = success
    
    # 2.6 Wrong OTP rejected
    r = requests.post(f"{BASE_URL}/api/auth/verify-reset-otp", json={
        "phone": "0711111111", "otp": "000000"
    })
...
def test_role_protection(FARMER_HEADERS, DEALER_HEADERS, NGO_HEADERS, ADMIN_HEADERS):
    ...
    print("\n" + "="*50)
    print("SECTION 3: ROLE PROTECTION")
    print("="*50)
    
    results = {}
    
    # 3.1 Farmer blocked from admin
    r = requests.get(f"{BASE_URL}/api/admin/stats", headers=FARMER_HEADERS)
    success = r.status_code == 403
    log_test("3.1 Farmer blocked from admin", success, f"Status: {r.status_code}")
    results["3.1"] = success
    
    # 3.2 Farmer blocked from dealer
    r = requests.get(f"{BASE_URL}/api/dealer/profile", headers=FARMER_HEADERS)
    success = r.status_code == 403
    log_test("3.2 Farmer blocked from dealer", success, f"Status: {r.status_code}")
    results["3.2"] = success
    
    # 3.3 Farmer blocked from NGO
    r = requests.get(f"{BASE_URL}/api/ngo/profile", headers=FARMER_HEADERS)
    success = r.status_code == 403
    log_test("3.3 Farmer blocked from NGO", success, f"Status: {r.status_code}")
    results["3.3"] = success
    
    # 3.4 Admin can access admin
    r = requests.get(f"{BASE_URL}/api/admin/stats", headers=ADMIN_HEADERS)
    success = r.status_code == 200
    log_test("3.4 Admin can access admin", success, f"Status: {r.status_code}")
    results["3.4"] = success
    
    # 3.5 No token returns 401
    r = requests.get(f"{BASE_URL}/api/farmers/profile")
    success = r.status_code == 401
    log_test("3.5 No token returns 401", success, f"Status: {r.status_code}")
    results["3.5"] = success
...
def test_ussd():
    ...
    log_test("4.1 USSD main menu", success, f"Status: {r.status_code}")
    results["4.1"] = success
    
    # 4.2 Has 5 options
    success = all(str(i) in response_text for i in range(1, 6))
    log_test("4.2 Main menu has 5 options", success)
    results["4.2"] = success
    ...
    log_test("4.3 Weather submenu", success, f"Status: {r.status_code}")
    results["4.3"] = success
    ...
    log_test("4.4 Today weather", success, f"Length: {len(resp)}")
    results["4.4"] = success
    ...
    log_test("4.5 Market prices", success)
    results["4.5"] = success
    ...
    log_test("4.6 Subscribe menu", success)
    results["4.6"] = success
...
def test_subscription_plans(FARMER_HEADERS):
    ...
    success = r.status_code == 200
    log_test("5.1 Get all plans", success, f"Status: {r.status_code}")
    results["5.1"] = success
    
    plans = r.json().get('data', [])
    log_test("5.2 Plans returned", len(plans) >= 4, f"Count: {len(plans)}")
    results["5.2"] = len(plans) >= 4
    
    # 5.3 Basic plan exists
    success = any(p.get('plan_id') == 'basic_monthly' and p.get('price_ksh') == 99 for p in plans)
    log_test("5.3 Basic plan KSH 99", success)
    results["5.3"] = success
    
    # 5.4 Pro plan exists
    success = any(p.get('plan_id') == 'pro_monthly' and p.get('price_ksh') == 299 for p in plans)
    log_test("5.4 Pro plan KSH 299", success)
    results["5.4"] = success
    
    # 5.5 Subscription status
    r = requests.get(f"{BASE_URL}/api/payments/subscription", headers=FARMER_HEADERS)
    success = r.status_code == 200
    log_test("5.5 Subscription status", success, f"Status: {r.status_code}")
    results["5.5"] = success
...
def test_agro_dealer(DEALER_HEADERS):
    ...
    success = r.status_code in [200, 201, 409]
    log_test("6.1 Create dealer profile", success, f"Status: {r.status_code}")
    results["6.1"] = success
    
    # 6.2 Get profile
    r = requests.get(f"{BASE_URL}/api/dealer/profile", headers=DEALER_HEADERS)
    success = r.status_code == 200
    log_test("6.2 Get dealer profile", success, f"Status: {r.status_code}")
    results["6.2"] = success
    
    # 6.3 Get stats
    r = requests.get(f"{BASE_URL}/api/dealer/stats", headers=DEALER_HEADERS)
    success = r.status_code == 200
    log_test("6.3 Dealer stats", success, f"Status: {r.status_code}")
    results["6.3"] = success
    ...
    log_test("6.4 Add product", success, f"Status: {r.status_code}")
    results["6.4"] = success
...
def test_ngo(NGO_HEADERS):
    ...
    success = r.status_code in [200, 201, 409]
    log_test("7.1 Create NGO profile", success, f"Status: {r.status_code}")
    results["7.1"] = success
    
    # 7.2 Get profile
    r = requests.get(f"{BASE_URL}/api/ngo/profile", headers=NGO_HEADERS)
    success = r.status_code == 200
    log_test("7.2 Get NGO profile", success, f"Status: {r.status_code}")
    results["7.2"] = success
    
    # 7.3 Dashboard
    r = requests.get(f"{BASE_URL}/api/ngo/dashboard", headers=NGO_HEADERS)
    success = r.status_code == 200
    log_test("7.3 NGO dashboard", success, f"Status: {r.status_code}")
    results["7.3"] = success
    
    # 7.4 Bulk registration
    ...
    log_test("7.4 Bulk registration", success, f"Status: {r.status_code}")
    results["7.4"] = success
...
def test_regression(FARMER_HEADERS, ADMIN_HEADERS):
    ...
    r = requests.get(f"{BASE_URL}/api/weather/forecast", 
                    params={"lat": -1.2921, "lon": 36.8219}, timeout=15)
    success = r.status_code == 200
    log_test("8.1 Weather forecast", success, f"Status: {r.status_code}")
    results["8.1"] = success
    
    # 8.2 Market prices
    r = requests.get(f"{BASE_URL}/api/market/prices")
    success = r.status_code == 200
    log_test("8.2 Market prices", success, f"Status: {r.status_code}")
    results["8.2"] = success
    
    # 8.3 Admin stats
    r = requests.get(f"{BASE_URL}/api/admin/stats", headers=ADMIN_HEADERS)
    success = r.status_code == 200
    log_test("8.3 Admin stats", success, f"Status: {r.status_code}")
    results["8.3"] = success
